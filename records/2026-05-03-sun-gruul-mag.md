...
**Druids (4):** Beaverfist, Gresac/Younea, Yxanb, Shapkica
**Paladins (3):** Heligeman, Thordrel, McJudgin
**Hunters (3):** Vaelruna, Roossy/Keatala, Grundiger
**Priests (3):** Kres/Dissi, Siljes, Boriest
...
- **CptKavior ↔ Gigakox spec swap (user directive).** Both signed up offspec; reverted to mainspec — CptKavior to Fury (was Prot), Gigakox to Prot (was Fury). Tank target 3/3 met without dual-spec flex.
...
- **Bench picks:**
  ...
  - **Greg (manual override)** — post-build swap with Siljes (see "Post-build roster changes" below). Priority-1 bench is manual-override only per `rules/02-bench-rotation.md`.
...
- **Post-build roster changes:**
  ...
  - **Greg ↔ Siljes bench-swap (manual override).** Siljes came off bench into roster as Shadow offspec DPS; Greg moved to bench. Greg is priority 1, so the swap is a discretionary user pick per `rules/02-bench-rotation.md` → "User's discretionary bench picks".
...
- **Raid-day changes** (McHughes no-show + Greg-bench user swap):
  ...
  - **Bench swap:** Siljes pulled into roster as Shadow offspec DPS; Greg moved to bench (priority-1 manual override).
  ...
  - **Cube clickers reshuffled (raid-day):** SE ← Kres/Dissi (was Greg); SW ← Siljes (was McHughes); NW ← Marino-Varthier (was Gigakox). S/NE unchanged.
