...
| Spot/Yorekbarn | 2        | 1                                         | manual override |
...
- **Bench picks:**
  ...
  - **Spot/Yorekbarn (manual override)** — Discord bench reaction.
...
- **Post-build roster changes:**
  ...
  - **Greg ↔ Siljes/Ejlis bench-swap (manual override).** Siljes/Ejlis came off bench into roster as Shadow offspec DPS; Greg moved to bench. Greg is priority 1, so the swap is a discretionary user pick per `rules/02-bench-rotation.md` → "User's discretionary bench picks".
...
- **Raid-day changes** (McHughes no-show + Greg-bench user swap):
  ...
  - **Bench swap:** Siljes/Ejlis pulled into roster as Shadow offspec DPS; Greg moved to bench (priority-1 manual override).
