...
| Mage Tank Healer     | Heligeman/Fugleman     |                                                    |
| Kiggler Tank         | Beaverfist, Vaelruna   | 2 ranged DPS; Beaverfist Balance offspec for this fight; Vaelruna joins after MDing Blindeye |
...
| South West  | Circle   | Siljes/Ejlis    |
...
| Spot/Yorekbarn | 2        | 1                                         | leader choice |

## Notes

- **New players (priority 2):** Nemajumarad (Warrior, mainspec Arms / offspec Tank), Boriest (Priest, mainspec Holy / offspec unknown). Recorded in `rules/04-players.md`.
- **Roossy/Keatala revealed Hunter alt** — BM DPS this raid; Druid character is Resto healer per past raids. `rules/04-players.md` updated; character-class mapping (which name = which class) not yet specified.
- **CptKavior ↔ Gigakox spec swap (leader directive).** Both signed up offspec; reverted to mainspec — CptKavior to Fury (was Prot), Gigakox to Prot (was Fury). Tank target 3/3 met without dual-spec flex.
...
- **Bench picks:**
  - **Ebonybolt (fair rotation)** — among 0-G+M-bench Enh Sham over-rep DPS; cross-location bench total tiebreaker (Ebonybolt 0 < Bergamotka 1).
  - **Spot/Yorekbarn (leader choice)** — Discord bench reaction.
  - **Greg (leader choice)** — post-build swap with Siljes/Ejlis (see "Post-build roster changes" below). Priority-1 bench is leader-choice only per `rules/02-bench-rotation.md`.
- **Encounter assignment swaps (leader override of strict table-order to preserve cross-role continuity):**
...
- **Post-build roster changes:**
  ...
  - **Grundiger walked in** to fill the no-show DPS slot — Hunter BM.
  - **Greg ↔ Siljes/Ejlis bench-swap (leader choice).** Siljes/Ejlis came off bench into roster as Shadow offspec DPS; Greg moved to bench. Greg is priority 1, so the swap is a discretionary leader pick per `rules/02-bench-rotation.md` → "Raid leader's discretionary bench picks".
  - **Beaverfist on Kiggler Tank co-slot in Balance offspec** for the Maulgar fight only; back to Resto for the rest of the night. Replaces Greg in the Kiggler 2-ranged-DPS pair.
  - **Cube clicker reshuffle** to absorb the Greg-bench, McHughes-no-show, and Siljes-promotion: SE ← Kres/Dissi (was Greg, now benched), SW ← Siljes/Ejlis (was McHughes, now withdrawn), NW ← Marino-Varthier (was Gigakox; Gigakox demoted to alt). S Jabbadhutt and NE CptKavior unchanged. Marino-Varthier no longer reserved for Mag MT — Ostbirger holds that reserve alone.

## Sanity check

**Verdict: YES** — sub-agent confirmed full rule compliance against the initial proposed roster. The two cross-role continuity asymmetries flagged at sub-agent time (Vaelruna Maulgar↔Blindeye MD, Gresac Kiggler↔Blindeye Healer) were resolved by the leader-directed swaps below.
...
- **Raid-day changes** (McHughes no-show + Greg-bench leader swap):
  - **Withdrawal:** McHughes (no-show). Warlocks-in-roster: 2 → 1 → Felhunter Subjugate dropped to single slot (Jabbadhutt).
  - **Walk-in:** Grundiger filled the vacated DPS slot (Hunter BM).
  - **Bench swap:** Siljes/Ejlis pulled into roster as Shadow offspec DPS; Greg moved to bench (priority-1 leader choice).
  - **Beaverfist mainspec change** (DPS Balance → Healer); recorded in `rules/04-players.md`.
  - **Kiggler Tank** (raid-day): Beaverfist (Balance offspec, this fight only) + Vaelruna (was Greg + Vaelruna).
  - **Cube clickers reshuffled (raid-day):** SE ← Kres/Dissi (was Greg); SW ← Siljes/Ejlis (was McHughes); NW ← Marino-Varthier (was Gigakox). S/NE unchanged.
