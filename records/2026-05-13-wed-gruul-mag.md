# Gruul's Lair + Magtheridon — Wednesday 13.05.2026
...
|------------------|----------|-------------------------------------------|-----------------|
| Adam(Kres/Dissi) | 1        | 2                                         | manual override |
...
|-------------------|------|----------------|------------------|--------------------------|---------------|
| Emil(Ostbirger)   | Tank | Paladin (Prot) | 4                | NW×3, NE×1               | 2026-05-10 NE |
...
On the bench: Adam(Kres/Dissi) and Pergatori. But if you show up online around raid time, there's a decent chance a spot will free up because of last-minute changes.

## Notes

- **Mark(Roossy/Keatala) rostered on his Keatala alt (Druid Resto), not his Hunter main** — the DPS pool was at its 17 ceiling when alts resolved at step 2 (CptKavior's Tier-0 tank flex already absorbed), so his Hunter-DPS profile didn't fit; only the Druid-Resto alt fit and was rostered per `rules/01-raid-compositions.md` → "Alts" (standing consent, distinct from comp flex). Steven(Gresac/Younea) (processed alphabetically first) kept his Shaman Elemental main because DPS still fit at 17 at his evaluation point.
- **Kiggler Tank: Guðjón(Jarðepli) (user override of raid-leader exclusion).** Under the day-of-week raid-leader rule then in effect, Guðjón(Jarðepli) (Wed RL) was excluded from all encounter roles per `rules/05-encounter-assignments.md` → "Raid leader exclusion"; the rule's algorithmic output was the 2-ranged-DPS fallback (Mathias(Vaelruna) + Steven(Gresac/Younea)). User overrode this on the night — Guðjón(Jarðepli) took Kiggler solo as the Balance druid.
