...
| **Comp flex** | A mid-step main → offspec move during roster construction — a player signs up mainspec, then the planner asks them to switch to offspec to fill a role gap. Specific procedures: **Dual-spec flex** (role-shortage), **Tank-surplus flex** (role-surplus). Distinct from **Offspec signup** (signup-time choice, see below). |
...
| **Offspec signup** | A signup-time choice: a player signs up as their offspec rather than mainspec (e.g., because their mainspec role is already at par). Distinct from **Comp flex**, which is a mid-step move during roster construction. No special handling under current rules — treated as a normal signup for selection and bench rotation. |
