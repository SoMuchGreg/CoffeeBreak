...
| **Split-location raid night** (or **split night**) | A single raid night attempting bosses from **two or more raid locations** — one roster, one bench, encounters drawn from each. Example: 3 first TK bosses then SSC's Lady Vashj. Canonical rule (definition, filename, title, composition target, bench tracking, encounter section): `rules/06-split-location-nights.md`. 25-man only. |
...
| **Target spec ranges** | The "Quick Reference: Number of Each Spec Typically Desired" table in `reference/raid-composition-guide.md` § 8 — the per-spec headcount ranges aimed for in any 25-man raid (e.g. Restoration Druid 1-2, BM Hunter 2-4). The per-spec form of the **composition target** (above); aspirational, not a hard cap. Used by the bench-rotation tiebreaker cascade (`rules/02-bench-rotation.md` → "Tiebreaker cascade"), which classifies a roster's count of each spec as over-/in-/under-range against it. |
...
| **Comp flex** | A mid-step main → offspec move during roster construction; for a role shortage it is **Resort 2** — the fallback after Resort 1, before Resort 3 (alt swap). Canonical rules: `rules/01-raid-compositions.md` → "Comp flex consent", "Handling role shortages" (Resort 1 / Resort 2 / Resort 3), and "Handling role surpluses". Distinct from **Offspec signup** (signup-time choice, see below). |
