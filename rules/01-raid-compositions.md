...
4. **Calibrate the role distribution to composition targets:**
...
**Timing.** Comp flex happens during step-4 calibration, after the headcount cut. Alt swap (Resort 3) also fires during step-4 calibration. An accepted flex or alt swap triggers the step 3–4 loop — see `rules/02-bench-rotation.md` → "Raid spot priority (selection order)", step 5.
