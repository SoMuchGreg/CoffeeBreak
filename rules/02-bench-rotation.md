...
The reservation acts during the headcount cut (steps 2–3); like any headcount-cut placement, a reserved member may afterwards be moved by step-5 role calibration (*Headcount cut vs. composition calibration* above).
...
- **Healer role group oversubscribed** → healer signups exceed the location's healer target — for 25-mans, **more than 5** (the fixed 25-man healer count; `rules/01-raid-compositions.md` → "25-man raids → General → Default composition"). Fair rotation fires within the Healer pool of the affected priority level(s).
