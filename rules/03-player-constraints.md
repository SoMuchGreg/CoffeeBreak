...
> **Currently paused.** Loot-conflict tracking is suspended for now — the Needlist is intentionally kept empty, so no loot conflicts surface in record files and the different-teams rule above never fires. Section header and Maintenance rules are retained for when tracking resumes.
