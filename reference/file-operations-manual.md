...
3. Apply fair bench rotation per `rules/02-bench-rotation.md` (full mechanism: "Bench groups", "Fairness requirement", "Mainspec priority within a pool (Mainspec-first rule)") using counts from `derived/bench-history-tbc.md`. Contingency triggers (rare): `rules/01-raid-compositions.md` → "Role placement: mainspec is authoritative".
