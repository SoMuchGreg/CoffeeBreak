...
1. **Read the signup count.** The header reads `X (+Y)`. **Use only X** — the count of on-time signups. **Ignore `(+Y)` entirely; it's Discord's aggregate of bench/tentative/late and is messy and unreliable.** Identify bench, tentative, and late players individually by the per-row visual indicator in the signup lists — never derive these states from `+Y`, and never reproduce `+Y` in the record file. Ignoring `+Y` is total: do not use it to cross-check the parse, infer that an off-screen bench/tentative/late signup exists, or ask the user to account for it — if no per-row indicator is visible, there is nothing to record, and a nonzero `+Y` with no visible status rows is normal, not a discrepancy.
...
| `derived/signup-stats-tbc.md` | **Update IF** the new/edited record file is in scope per that file's **Scope** section (TBC-era record files). See its **Maintenance** section for the full delta logic — in brief: apply per-player Signups deltas, record First signup and **Rank** (from `rules/04-players.md`) for new rows, recompute Signup rate total for every row whose Raids-in-window changed and Signup rate recent for every row, recompute Last signed up X days ago for every row, refresh the "Computed as of" header, re-sort by Signup rate total desc (alphabetical tiebreak), renumber. Former players are excluded. Skip entirely for out-of-scope record files (currently any old-world record file). |
...
| `derived/signup-stats-tbc.md` | Only if the record file is in-scope (TBC-era, per that file's Scope section) **and** the increment was previously applied (post-build withdrawal). | Decrement the player's `Signups` by 1. If `Signups` hits 0, remove the row. Otherwise, if this record file was the player's `First signup`, recompute `First signup` to the next-earliest in-scope record file containing them and recompute `Raids-in-window` + `Signup rate total` for their row; if not their first, `Raids-in-window` is unchanged and only `Signup rate total` recomputes. If the withdrawn record file falls within the current 10-raid window, also recompute `Signup rate recent` for their row. Additionally, if this record file was the player's most recent in-scope signup file, recompute `Last signed up X days ago` from the next-most-recent in-scope record file containing them in `## Signups`. Re-sort by `Signup rate total` desc (alphabetical tiebreak), renumber `#`, refresh the `Computed as of` header. |
...
> **Verify after applying.** Withdrawals (and no-shows, which defer to this event) are the most common source of stale `signup-stats-tbc.md` columns — a `Signups` left un-decremented, or a `Last signed up` still pointing at the withdrawn raid. After updating the derived files, run the invariants at `derived/signup-stats-tbc.md` → Maintenance → **Consistency checks**.
...
| `derived/signup-stats-tbc.md` | Same trigger as `signup-history-total.md`, scoped to in-scope record files (see that file's Scope section). If only the roster changed but Signups didn't, no update. | Same delta mechanics plus Raids-in-window / Signup rate total / Signup rate recent / Last signed up X days ago recompute + "Computed as of" header. |
...
| `records/*.md` | Update every historical record file that references the old name, wherever it appears (signup lists, roster tables, bench tables, Notes sections). A pure name normalization doesn't violate the record-files-are-immutable principle — it updates the label without changing any factual content. **Where the old name records a fact rather than referencing the player, renaming it falsifies the record — leave it:** historical naming-event notes (*"signed up as X"*, *"renamed from Y to X"*) and explicit character-name references (*"the X character"*). A bullet's present-day subject still normalizes (e.g. `Saskia(Siljes)`); only the historical or character token keeps the old name. A verbatim user-instruction quote *may* be canonicalized — the renamed name still points to the same person, so the instruction's meaning is unchanged. |

### Afterwards:
- **Verify completeness** by running `Grep <old_name>` across the whole project. Legitimate remaining hits: the optional alias note in `rules/04-players.md` (if you added one), and any historical-fact token deliberately preserved per the `records/*.md` row above. Any other hit is a missed reference that needs fixing.
- **Realign edited tables.** A new canonical name changes cell widths, so re-pad every **aligned-grid** table the rename touched — each cell padded to its column's widest entry so the pipes line up again. Count **display width** (characters), not bytes: names like `Guðjón(Jarðepli)` are multibyte but one column wide, so byte-based padding misaligns them. This covers the player rosters, the `derived/` tables, the Needlist/enchanters/availability/must-be-together tables, the encounter-role tables, and every record-file roster/bench/loot/encounter table. **Prose tables** — cells holding long free-text that were never column-padded (e.g. `config/project.md`'s Terminology glossary, the record-template instruction tables) — keep their existing single-space style; only the names inside their cells change.
...
INPUTS for generating a record file:
  ...
  └── derived/signup-stats-tbc.md  ← combined signup count, signup rates (total + recent), and last-signup recency (days), TBC-era record files only (statistic only)
...
OUTPUTS:
  ...
  └── derived/signup-stats-tbc.md  ← same, but only for TBC-era record files; also recomputes Signup rate total, Signup rate recent, and Last signed up X days ago
