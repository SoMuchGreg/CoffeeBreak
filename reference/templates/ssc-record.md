...
> {Small announcement, e.g. "Our first steps into T5! Bring your consumables ;)"}    <!-- Canonical rules — required, consumables reminder, Discord mirror: reference/file-operations-manual.md → "Writing the small announcement". -->
...
**Composition check:** Target {target} for SSC (per `rules/01-raid-compositions.md`). Actual: {T}/{H}/{DPS} = {total}. Status: ✅ / ⚠️ {explanation if outside target, e.g. "4 healers — 1 below the 5-healer target, no flex accepted" or "6 healers, ran by user override"}.
