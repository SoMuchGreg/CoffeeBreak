...
- **Always write the full name "target spec ranges" — never a bare "§8" / "Section 8".** The per-spec desired-headcount-range table (`reference/raid-composition-guide.md` §8) has a canonical name — **target spec ranges** (`config/project.md` → Terminology) — and you **must** use it in every human-facing context: chat replies and record-file prose alike (`## Composition check`, `## Notes`, `## Sanity check`). A bare "§8" or "Section 8" is **never** acceptable as the table's name; the abbreviations are cryptic. The only permitted bare use is as a strict **location pointer** inside a technical cross-reference that names the guide section — e.g. "(`reference/raid-composition-guide.md` § 8)" — a navigation coordinate, not a stand-in for the name. Mandatory; not relaxed by brevity (see the "Be brief" rule above).
