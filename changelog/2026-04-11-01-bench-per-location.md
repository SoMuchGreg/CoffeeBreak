...
## Change
`rules/02-bench-rotation.md` — bench fairness is now tracked per raid location (each raid type independently), not as a single overall count. `derived/bench-history-tbc.md` was restructured into separate per-raid-location sections.
